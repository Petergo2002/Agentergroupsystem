...
import { PDFStructureBuilder } from "./pdf-structure-builder";
...
export function RapportSettings() {
  ...
  const { profile: pdfProfile, setProfile: setPdfProfile } = usePdfProfileStore();
  ...
  const [newSection, setNewSection] = useState({ 
    title: "", 
    description: "", 
    type: "text" as "text" | "image" | "chart" | "image_gallery" | "image_annotated",
    imageUrl: "",
    imageAltText: "",
  ...
  const handleToggleSectionForTemplate = async (
  ...
  const handleReorderTemplateSection = async (
    sectionId: string,
    direction: "up" | "down",
  ) => {
    if (!selectedTemplate) return;
  ...
  const handleAddSectionToLibrary = async () => {
    ...
    await createReportSectionRecord({
      ...
      imageUrl: newSection.type === "image" ? newSection.imageUrl.trim() : undefined,
      imageAltText: newSection.type === "image" ? newSection.imageAltText.trim() : undefined,
  ...
  return (
    <div className="flex h-[calc(100vh-theme(spacing.16))] flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Rapportinställningar</h2>
      ...
      <div className="grid flex-1 gap-8 md:grid-cols-[200px_1fr]">
        ...
        <aside className="flex flex-col gap-2">
          <h3 className="mb-2 px-2 text-xs font-medium uppercase text-muted-foreground">Designer</h3>
          <Button
