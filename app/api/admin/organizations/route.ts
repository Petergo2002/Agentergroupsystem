...
export async function POST(request: NextRequest) {
  try {
    ...
    if (flagsError) {
      logger.error("Error creating feature flags", { error: flagsError });
    ...
    if (user_email && user_first_name && user_last_name && user_password) {
      try {
        ...
        logger.info("Organization setup complete", { email: user_email });
    ...
    logger.error("Error in POST /api/admin/organizations", { error });
    return NextResponse.json(
