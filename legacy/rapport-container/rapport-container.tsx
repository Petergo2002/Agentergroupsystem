...
export function RapportContainer({ mode = "new" }: RapportContainerProps) {
  ...
  const _handleExportReport = async () => {
  ...
  const _draftReports = reports.filter((r) => !r.exportedAt);
                      ...
                      <div className="rounded-2xl border p-4 space-y-3">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {selectedReport.sections.length} st
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">
                          ...
                          <span className="font-medium">
                            {
                              selectedReport.sections.filter(
                        ...
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {selectedReport.checklist.length} st
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">Bilagor</span>
                          <span className="font-medium">
                            {selectedReport.assets.length} st
                          </span>
                        </div>
                      </div>
...
function NewReportWizard({
  ...
  const _buildPreviewReport = (): Report => {
    return {
      id: "preview",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        const convertedGannTable = gannTableImage
          ? (await convertImagesToBase64([gannTableImage]))[0]
          : undefined;
  ...
  const _handleExportCreatedReport = async () => {
    if (!createdReport) return;

    try {
      const template =
        templates.find(
          (template) => template.id === createdReport.templateId,
        ) ??
        selectedTemplate ??
        null;
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={async (e) => {
                        ...
                        if (
                          confirm(
                            `Är du säker på att du vill ta bort "${report.title}"?`,
                          )
                        ) {
                          try {
              ...
              <Card key={section.id}>
                ...
                <CardContent>
                  {section.type === "image" && section.imageUrl ? (
                    ...
                    <div className="space-y-4">
                      {section.annotatedImages &&
                      section.annotatedImages.length > 0
                                    ...
                                    prev.map((s) =>
                                      s.id === section.id
                                        ? {
                                            ...s,
                                            annotatedImages:
                                              s.annotatedImages?.map((i) =>
                                                i.id === img.id
                                                  ? {
                                                      ...i,
                                                      annotationData: shapes,
                                                    }
                                                  : i,
                                              ),
                                          }
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            input.onchange = (e) => {
                              ...
                              toast.success(
                                `${files.length} bild(er) uppladdade`,
                              );
                            };
                            input.click();
                          }}
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Din rapport är skapad. Granska innehållet nedan och ladda ner
                  när du är klar.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-2 text-xs rounded-lg border bg-emerald-50 border-emerald-200 p-3">
                  ...
                  <span className="text-emerald-800">
                    Rapporten är sparad som utkast. Du kan fortsätta redigera
                    den senare.
                  </span>
                </div>
              </CardContent>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {currentStep.key === "final" && createdReport ? (
              ...
              <Button
                onClick={() => {
                  if (pdfUrl) {
                    ...
                    window.open(
                      `/api/reports/${createdReport.id}/pdf`,
                      "_blank",
                    );
                  }
                }}
            ...
            <Button
              onClick={
                currentStep.key === "sections" ? handleSubmit : handleNext
              }
              disabled={
...
export function buildPrintableHtml(
  ...
  const _showInternalInfo =
    viewMode === "internal" || profile.displayInternalNotes;
  ...
  const sectionHtml = report.sections
    .map((section) => {
      ...
      if (section.type === "image_annotated" && section.assetId) {
        ...
        const imageUrl =
          section.annotatedImageUrl || asset?.url || section.assetId;
