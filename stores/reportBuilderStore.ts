  ...
  persist(
    (set, get) => ({
      ...
      updateFormData: (data) => {
        ...
        if (templateBindings) {
          const textFields = ['inledning', 'bakgrund', 'matmetoder', 'slutsats'] as const;
          
          textFields.forEach(field => {
            // Om fältet inte har manuell override och vi har source text
            if (!templateBindings.manualOverrides[field] && templateBindings.source[field]) {
              // Re-resolve template text
              const resolved = resolveTemplateText(
                templateBindings.source[field], 
                newFormData
              );
              (newFormData as any)[field] = resolved;
      ...
      applyTemplate: (template) => {
        ...
        Object.keys(bindings.source).forEach(field => {
          ...
          if (sourceText) {
            bindings.resolved[field] = resolveTemplateText(sourceText, formData);
          }
        });
      ...
      clearDraft: () => {
        ...
        lockedFields.forEach(field => {
          if (field in formData) {
            (clearedData as any)[field] = (formData as any)[field];
          }
        });
